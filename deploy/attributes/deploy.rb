...
case node[:platform]
when 'debian','ubuntu'
  default[:opsworks][:deploy_user][:group] = 'opsworks'
when 'centos','redhat','fedora','amazon'
  default[:opsworks][:deploy_user][:group] = node['opsworks']['rails_stack']['name'] == 'nginx_unicorn' ? 'nginx' : 'opsworks'
end
...
node[:deploy].each do |application, deploy|
  ...
  default[:deploy][application][:code_root] = ''
  if deploy[:code_root]
    default[:deploy][application][:absolute_code_root] = "#{default[:deploy][application][:current_path]}/#{deploy[:code_root]}"
  else
    default[:deploy][application][:absolute_code_root] = "#{default[:deploy][application][:current_path]}"
  end

  if default[application][:document_root]
    default[:deploy][application][:absolute_document_root] = "#{default[:deploy][application][:absolute_code_root]}/#{default[application][:document_root]}/"
  else
    default[:deploy][application][:absolute_document_root] = "#{default[:deploy][application][:absolute_code_root]}/"
  end
