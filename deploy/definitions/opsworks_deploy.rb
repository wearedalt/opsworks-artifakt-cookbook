define :opsworks_deploy do
  ...
  display_title do
    title 'Removing old failed releases'
  end

  execute 'Remove failed releases' do
    only_if { Dir.exist?("#{node[:deploy][application][:deploy_to]}/releases/") }
    user 'root'
    cwd "#{node[:deploy][application][:deploy_to]}/releases/"
    command "find . -type f -iname .arkt_processing_flag -print0 | xargs -0 --no-run-if-empty dirname | xargs --no-run-if-empty rm -rf"
  end
  
  display_title do
    title 'Gettings source code'
  end

  display_text do
    text "Repository Url: #{deploy[:scm][:repository]}"
  end
  display_text do
    text "Branch: #{deploy[:scm][:revision]}"
  end
  ...
  if deploy[:scm] && deploy[:scm][:scm_type] != 'other'
    ...
    deploy deploy[:deploy_to] do
      ...
      before_migrate do
        ...
        file "#{release_path}/.arkt_processing_flag" do
          content ''
        end
        ...
        # run artifakt deploy
        node.set['app_user'] = node[:deploy][application][:user]
        node.set['app_group'] = node[:deploy][application][:group]
        node.set['app_name'] = application
        if node[:deploy][application][:code_root]
          node.set['app_release_path'] = "#{release_path}/#{node[:deploy][application][:code_root]}"
          node.set['short_release_path'] = "#{release_path}"
        else
          node.set['app_release_path'] = "#{release_path}"
        end


        artifakt_global_deploy
  ...
  template "/etc/logrotate.d/opsworks_app_#{application}" do
    ...
    group "root"
    mode 0644
    variables( :log_dirs => node[:logrotate][:log_dirs] )
  end
end
